...
def process_user_directory(user_dir):
    ...
    # Log that we found files in this directory
    ...
    for filename in files:
        ...
        if not year or not doy:
            logging.warning(f"Could not determine year/doy from filename: {filename}")
            continue
...
def main():
    ...
    # Check if base RINEX directory exists
    if not os.path.exists(BASE_RINEX_DIR):
        try:
            os.makedirs(BASE_RINEX_DIR)
        except Exception as e:
            logging.error(f"Failed to create base RINEX directory: {e}")
            sys.exit(1)
    
    # Process each user directory
    try:
        user_dirs = [d for d in os.listdir(HOMES_DIR) 
                    if os.path.isdir(os.path.join(HOMES_DIR, d))]
    except Exception as e:
        logging.error(f"Error reading homes directory: {e}")
        sys.exit(1)
